...
public class Trie {
     ...
     * 
     * @param ascii
     *            是否是Ascii树（如果设为true，则会针对ascii加速，否则会支持Unicode）
     */
    ...
    private void removePartialMatches(String searchText,
        ...
        for (Emit emit : collectedEmits) {
            if ((emit.getStart() == 0 || !Character
                    .isAlphabetic(searchText.charAt(emit.getStart() - 1)))
                    && (emit.getEnd() + 1 == size || !Character.isAlphabetic(
     ...
     * 
     * @param position
     *            当前位置，也就是匹配到的模式串的结束位置+1
     * @param currentState
     *            当前状态
     * @param collectedEmits
     *            保存位置
     */
    private static void storeEmits(int position, State currentState,
            List<Emit> collectedEmits) {
        Collection<String> emits = currentState.emit();
        if (emits != null && !emits.isEmpty()) {
            for (String emit : emits) {
                collectedEmits.add(
                        new Emit(position - emit.length() + 1, position, emit));
            }
        }
    }
